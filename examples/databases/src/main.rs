...
fn main() {
    ...
    let sqlite_pool = init_pool(sqlite_manager, max_size).unwrap();

    // Diesel Postgresql
    let pg_manager = ConnectionManager::<PgConnection>::new(postgres_url);
    let diesel_pool = init_pool(pg_manager, max_size).unwrap();

    // Rediss
    let redis_manager = RedisConnectionManager::new(&redis_url[..]).unwrap();
    let redis_pool = init_pool(redis_manager, max_size).unwrap();
