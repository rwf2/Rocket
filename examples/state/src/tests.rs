...
fn test_raw_state_count() {
    ...
    assert_eq!(count(State::from(inspector).unwrap()), "0");
    assert!(index(State::from(inspector).unwrap()).0.contains("Visits: 1"));
    assert_eq!(count(State::from(inspector).unwrap()), "1");
}
