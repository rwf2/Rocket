...
/// ```
#[derive(Debug, Clone)]
pub struct AcceptEncoding(pub(crate) SmallVec<[QContentCoding; 1]>);
    ...
    ($($name:ident ($check:ident): $str:expr, $c:expr,)+) => {
        $(
            ...
            pub const $name: AcceptEncoding = AcceptEncoding(
                SmallVec::from_const([QContentCoding(ContentCoding::$name, None)])
            );
         )+
    };
}
...
impl AcceptEncoding {
    ...
    /// ```
    #[inline(always)]
    pub fn new<T: IntoCollection<QContentCoding>>(items: T) -> AcceptEncoding {
        AcceptEncoding(items.into_collection())
    }
...
impl<T: IntoCollection<ContentCoding>> From<T> for AcceptEncoding {
    #[inline(always)]
    fn from(items: T) -> AcceptEncoding {
        AcceptEncoding(items.mapped(|item| item.into()))
    }
}
