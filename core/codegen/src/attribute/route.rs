            ...
            if let Some(previous) = set.replace(segment) {
                diags.push(
                    span.error(format!("duplicate parameter: `{}`", previous.name))
                        .span_note(
                            previous.span,
                            "previous parameter with the same name here",
                        ),
                )
            }
        }
    ...
    for input in &function.decl.inputs {
        ...
        let (ident, ty) = match input {
            syn::FnArg::Captured(arg) => match arg.pat {
                ...
                syn::Pat::Wild(_) => {
                    diags.push(
                        span.error("handler arguments cannot be ignored").help(help),
                    );
                    continue;
                }
                _ => {
...
fn incomplete_route(
    ...
    let function: syn::ItemFn = syn::parse(input).map_err(syn_to_diag).map_err(|d| {
        d.help(format!("#[{}] can only be used on functions", method_str))
    })?;
