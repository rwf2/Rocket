...
where
    T: FromForm<'f, Error = FormParseError<'f>>,
{
...
fn simple() {
    ...
    let task: Option<TodoTask> =
        strict("_method=patch&description=Hello&completed=off").ok();
    assert_eq!(
        task,
        Some(TodoTask {
            description: "Hello".to_string(),
...
fn generics() {
    ...
    let form_string =
        &["base=just%20a%20test", "a=hey%20there", "b=a", "c=811"].join("&");
...
fn form_errors() {
    ...
    assert_eq!(
        form,
        Err(FormParseError::BadValue("other".into(), "unknown".into()))
    );

    let form: Result<WhoopsForm, _> = strict("complete=unknown&other=unknown");
    assert_eq!(
        form,
        Err(FormParseError::BadValue(
            "complete".into(),
            "unknown".into()
        ))
    );
    ...
    assert_eq!(
        form,
        Err(FormParseError::Unknown("extra".into(), "foo".into()))
    );
    ...
    assert_eq!(
        form,
        Err(FormParseError::Unknown("unknown".into(), "foo".into()))
    );
