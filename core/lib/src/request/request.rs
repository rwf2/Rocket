...
impl<'r> Request<'r> {
    ...
    pub(crate) fn from_hyp(
    ...
    ) -> Result<Request<'r>, (Error<'r>, Option<ConnectionMeta>)> {
        // Ensure that the method is known. TODO: Allow made-up methods?
        let method = if let Some(method) = Method::from_hyp(&hyper.method) {
            method
        } else {
            return Err((Error::BadMethod(&hyper.method), connection));
        };
        ...
        let uri = if let Some(uri) = hyper.uri.path_and_query() {
            ...
            return Err((Error::InvalidUri(&hyper.uri), connection));
        };
