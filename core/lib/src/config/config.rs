...
impl Config {
    ...
    pub fn set_secret_key<K: Into<String>>(&mut self, key: K) -> Result<()> {
        let key = key.into();
        let error = self.bad_type("secret_key", "string", "a 256-bit base64 encoded string");
    ...
    pub fn set_tls(&mut self, certs_path: &str, key_path: &str) -> Result<()> {
        ...
        let certs = util::load_certs(self.root_relative(certs_path)).map_err(|e| match e {
            Error::Io(e) => ConfigError::Io(e, "tls.certs"),
            _ => self.bad_type("tls", pem_err, "a valid certificates file"),
        })?;

        // And now the private key.
        let key = util::load_private_key(self.root_relative(key_path)).map_err(|e| match e {
