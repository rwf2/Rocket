...
impl<'c> LocalRequest<'c> {
    ...
    pub(crate) fn new(
        ...
        let request = Request::new(client.rocket().0, method, Origin::dummy());
    ...
    async fn _dispatch(
        ...
        if let Ok(uri) = maybe_uri {
            ...
            let res = client.rocket().0.handle_error(Status::BadRequest, request).await;
            return LocalResponse { _request: owned_request, response: res };
        }

        // Actually dispatch the request.
        let response = client.rocket().0.dispatch(request, Data::local(data)).await;
