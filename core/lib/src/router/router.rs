...
mod test {
    ...
    where
        I: Iterator<Item = (Option<isize>, &'static str)>,
    {
    ...
    fn test_ok_routing() {
        ...
        router
            .routes
            .push(Route::new(Post, "/hello", dummy_handler));
        router
            .routes
            .push(Route::new(Delete, "/hello", dummy_handler));
    ...
    fn router_with_catchers(catchers: &[(Option<u16>, &str)]) -> Result<Router<Finalized>> {
        ...
        for (code, base) in catchers {
            ...
            router
                .catchers
                .push(catcher.map_base(|_| base.to_string()).unwrap());
        }
